...
content: 
  ...
  #    * frp terminates TCP on both ends, so there is no packet-in-packet
  #      overhead -- but the *end user -> Iran server* path very often has an
  #      MTU below 1500 (mobile carriers, PPPoE, another tunnel on the user's
  #      device) while ICMP "fragmentation needed" is filtered somewhere.
  #    * The 3-way handshake and small responses fit, so the site "connects".
  #      The first 1460-byte data segment is dropped without notice, the server
  #      retransmits the same oversized segment forever, and the page hangs.
  #      Small/simple sites load, heavy sites never finish. Exactly the symptom.
  #    * Clamping the MSS we advertise (OUTPUT) and the MSS we accept from the
  #      peer (PREROUTING) makes every segment small enough to survive the path.
  #  IPv6 uses a 40-byte header instead of 20, so its MSS is 20 lower.
  # -----------------------------------------------------------------------------
  write_mss_script() {
  ...
  # Probe the real path MTU to a host and translate it into a safe MSS.
  probe_mss() {
      ...
      for payload in 1472 1452 1432 1412 1392 1372 1352 1332 1312 1272 1232 1172; do
          if ping -c 1 -W 2 -M do -s "$payload" "$host" >/dev/null 2>&1; then
              mtu=$(( payload + 28 ))
              # 40 bytes of IPv4+TCP header, then 20 bytes of headroom for the
              # user's own last mile which we cannot measure from here.
              echo $(( mtu - 60 ))
              return
          fi
      done
  ...
  # ---------------- broken IPv6 on the exit node (AAAA sites hang) ------------
  fix_broken_ipv6() {
  ...
  # =============================================================================
  #  CONFIG BLOCKS  (frp v1 TOML -- TCP ONLY)
  # =============================================================================
  ...
  # Strip every key this script manages (including legacy non-TCP keys) so an old
  # install can be migrated cleanly. transport.tls.serverName is deliberately
  # preserved -- that is the user's domain-fronting/SNI choice.
  strip_managed_keys() {
      local file="$1"
      ...
      # drop our own previous comment banners so they do not accumulate
  ...
  Restart=always
  RestartSec=5
  # Never let systemd's start-rate limiter park the service in a failed state
  # after a burst of restarts -- that turned a 30-second outage into a permanent
  ...
  # How long frps waits for frpc to hand over a work connection. The default of
  # 10s is too tight for a lossy intercontinental hop and shows up as random
  # "connection reset" on the user side.
  ...
  # =============================================================================
  #  INSTALL: FOREIGN SERVER (frpc)  --  TCP ONLY
  # =============================================================================
  install_client() {
  ...
  # Keep retrying forever instead of exiting on a bad login: during a temporary
  # block the service must come back on its own, not stay dead until a reboot.
  loginFailExit = false
  ...
  # =============================================================================
  #  REPAIR / MIGRATE AN EXISTING INSTALL
  # =============================================================================
  repair_install() {
      ...
      if [ -f "${CONF_DIR}/frpc.toml" ]; then
          ...
          # transport keys must sit above the [[proxies]] tables
          local tmp; tmp="$(mktemp)"
          awk -v RS='' 'NR==1' /dev/null >/dev/null 2>&1  # no-op, keeps awk happy on busybox
          sed -n '1,/^\[\[proxies\]\]/p' "${CONF_DIR}/frpc.toml" | sed '$d' > "$tmp"
          client_transport_block "$pool" >> "$tmp"
          sed -n '/^\[\[proxies\]\]/,$p' "${CONF_DIR}/frpc.toml" >> "$tmp"
          # if the file had no proxies at all, the split above produced nothing
  ...
  # =============================================================================
  #  DIAGNOSE: "the tunnel is up but some sites don't load"
  # =============================================================================
  diagnose() {
